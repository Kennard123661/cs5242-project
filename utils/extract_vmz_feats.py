...
from facebook_vmz.lib.models import model_builder
from facebook_vmz.lib.utils import model_helper
from facebook_vmz.lib.utils import model_loader
from facebook_vmz.lib.utils import metric
from facebook_vmz.lib.utils import reader_utils
import data.vmz_data as vmz_data
...
def main():
    ...
    parser.add_argument("--db_type", type=str, default='pickle', help="Db type of the testing model")
    parser.add_argument("--model_name", type=str, required=True, help="Model name")
    parser.add_argument("--model_depth", type=int, required=True, help="Model depth")
    parser.add_argument("--gpus", type=str, default=None, help="Comma separated list of GPU devices to use")
    parser.add_argument("--num_gpus", type=int, default=torch.cuda.device_count(),
                        help="Number of GPU devices (instead of --gpus)")
    parser.add_argument("--scale_h", type=int, required=True, help="Scale image height to")
    parser.add_argument("--scale_w", type=int, required=True, help="Scale image width to")
    parser.add_argument("--crop_size", type=int, required=True, help="Input image size (to crop to)")
    parser.add_argument("--clip_length_rgb", type=int, required=True, help="Length of input clips")
    parser.add_argument("--sampling_rate_rgb", type=int, required=True, help="Frame sampling rate")
    parser.add_argument("--num_labels", type=int, default=400, help="Number of labels")
    parser.add_argument("--num_channels", type=int, default=3, help="Number of channels")
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size, per-GPU")
    parser.add_argument("--load_model_path", type=str, default='', required=True,
    ...
    parser.add_argument("--output_path", type=str, required=True,
                        help="Path to output pickle; defaults to features.pickle next to <test_data>")
    parser.add_argument("--features", type=str, default="final_avg", help="Comma-separated list of blob names to fetch")
    parser.add_argument("--num_decode_threads", type=int, default=4, help="")

    parser.add_argument("--clip_length_of", type=int, default=8, help="Frames of optical flow data")
    parser.add_argument("--sampling_rate_of", type=int, default=2, help="Sampling rate for optial flows")
    parser.add_argument("--frame_gap_of", type=int, default=2, help="Frame gap of optical flows")
    parser.add_argument("--input_type", type=int, default=0, help="0=rgb, 1=optical flow")
    parser.add_argument("--flow_data_type", type=int, default=0, help="0=Flow2C, 1=Flow3C, 2=FlowWithGray, 3=FlowWithRGB")
    parser.add_argument("--do_flow_aggregation", type=int, default=0,
                        help="whether to aggregate optical flow across multiple frames")

    parser.add_argument("--clip_per_video", type=int, default=1, help="When clips_per_video > 1, sample this many "
    ...
    parser.add_argument("--channel_multiplier", type=float, default=1.0, help="Channel multiplier")
    parser.add_argument("--use_dropout", type=int, default=0, help="Use dropout at the prediction layer")
    parser.add_argument("--save_h5", type=bool, default=0, help="whether save to h5 or pickle, default to pickle")
    parser.add_argument("--bottleneck_multiplier", type=float, default=1.0, help="Bottleneck multiplier")
    parser.add_argument("--get_start_frame", type=bool, default=0, help="Output clip start frame")
